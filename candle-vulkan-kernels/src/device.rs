...
use gpu_allocator::vulkan::{Allocator, AllocatorCreateDesc, VmaAllocationCreateInfo, VmaPoolCreateFlags};
...
impl VulkanContext {
    ...
    fn create_instance(entry: &Entry) -> Result<Instance> {
        ...
        let mut extension_names = vec![
            ash::vk::KhrSurfaceExtension::name().as_ptr(),
            ash::vk::KhrWin32SurfaceExtension::name().as_ptr(),
        ];

        let layer_names = vec![
            CString::new("VK_LAYER_KHRONOS_validation")?.as_ptr(),
        ];
    ...
    fn create_device(
        ...
        let compute_queue_family = queue_family_properties
            ...
            .ok_or(VulkanError::Message("No compute queue family found".into()))? as u32;
    ...
    fn create_allocator(
        ...
        let create_desc = AllocatorCreateDesc {
            ...
            preferred_device_type: gpu_allocator::vulkan::DeviceMemoryType::Any,
    ...
    fn query_device_info(instance: &Instance, physical_device: vk::PhysicalDevice) -> Result<DeviceInfo> {
        ...
        let subgroup_properties = unsafe {
            instance.get_physical_device_subgroup_properties(physical_device)
        };
