...
//!
//! Includes both f32 matvec and quantized matvec for various formats.
//! Supports fused dequantization for Q4_0, Q4_1, Q5_0, Q5_1, Q8_0, and Q4_K.
...
/// Quantization type for selecting the appropriate kernel
...
impl QuantType {
    ...
    pub fn block_size(&self) -> usize {
        match self {
            ...
            QuantType::Q4_1 => Q4_1_BLOCK_SIZE,
            QuantType::Q5_0 => Q5_0_BLOCK_SIZE,
            QuantType::Q5_1 => Q5_1_BLOCK_SIZE,
            QuantType::Q8_0 => Q8_0_BLOCK_SIZE,
            QuantType::Q4K => Q4K_BLOCK_SIZE,
        }
    }

    /// Get the shader source for this quantization type
    fn shader_source(&self) -> &'static [u8] {
        match self {
            QuantType::Q4_0 => source::MUL_MAT_VEC_Q4_0_F32,
            QuantType::Q4_1 => source::MUL_MAT_VEC_Q4_1_F32,
            QuantType::Q5_0 => source::MUL_MAT_VEC_Q5_0_F32,
            QuantType::Q5_1 => source::MUL_MAT_VEC_Q5_1_F32,
            QuantType::Q8_0 => source::MUL_MAT_VEC_Q8_0_F32,
            QuantType::Q4K => source::MUL_MAT_VEC_Q4_K_F32,
        }
    }

    /// Get the pipeline name for this quantization type
    fn pipeline_name(&self) -> &'static str {
        match self {
            QuantType::Q4_0 => "mul_mat_vec_q4_0_f32",
            QuantType::Q4_1 => "mul_mat_vec_q4_1_f32",
            QuantType::Q5_0 => "mul_mat_vec_q5_0_f32",
            QuantType::Q5_1 => "mul_mat_vec_q5_1_f32",
            QuantType::Q8_0 => "mul_mat_vec_q8_0_f32",
            QuantType::Q4K => "mul_mat_vec_q4_k_f32",
        }
    }
}
...
pub fn call_mul_mat_vec_q4k(
    ...
    assert!(
        ncols % Q4K_BLOCK_SIZE == 0,
        "ncols must be multiple of Q4K block size (256)"
    );
...
/// Execute batched quantized matrix-vector multiplication for any supported quant type
///
/// This is the generic entry point that dispatches to the appropriate kernel based on
/// the quantization type. All quantized formats use the same push constant structure.
///
/// For each batch: output[batch] = quant_matrix @ vector[batch]
///
/// # Arguments
/// * `kernels` - Kernel manager
/// * `command_buffer` - Command buffer to record into
/// * `quant_type` - The quantization format of the matrix
/// * `batch_size` - Number of batches
/// * `nrows` - Number of rows in matrix (output dimension, n)
/// * `ncols` - Number of columns in matrix (k, must be multiple of block size)
/// * `quant_matrix` - Quantized matrix buffer [nrows x ncols]
/// * `vector` - Input f32 vector buffer [batch_size x ncols]
/// * `vector_offset` - Byte offset into vector buffer
/// * `output` - Output f32 vector buffer [batch_size x nrows]
pub fn call_mul_mat_vec_quant(
    kernels: &Kernels,
    command_buffer: vk::CommandBuffer,
    ...
    assert!(
        ncols % block_size == 0,
        "ncols ({}) must be multiple of block size ({}) for {:?}",
        ncols,
        block_size,
        quant_type
    );
    ...
    // Load pipeline for this quant type
    let cached = kernels.load_pipeline(
        quant_type.pipeline_name(),
        quant_type.shader_source(),
        3,
        std::mem::size_of::<MulMatVecQ4KParams>() as u32,
    )?;
    ...
    // Push constants - same structure for all quant types
    let params = MulMatVecQ4KParams {
        ncols: ncols as u32,
        stride_a: ncols as u32,
        stride_b: ncols as u32,
